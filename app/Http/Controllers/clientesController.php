...
class clientesController extends Controller
{
    ...
    public function transferir(Request $request){
        ...
        if($request->cuenta == \Session::get('cuenta')){
            flash('ERROR: No puede realizar transferencia a su propia cuenta.')->error()->important();
            return redirect()->back()->withInput();
        }
        ...
        $saldo_actual = DB::table('cuenta')->where('num_cuenta','=',\Session::get('cuenta'))->value('saldo');

        if($saldo_actual < $request->monto){
            flash('ERROR: El monto es mayor a su saldo actual, intente de nuevo.')->error()->important();
            return redirect()->back()->withInput();   
        }

        $hora_fecha = Carbon::now();
        $nuevo_saldo=$cuenta[0]->saldo + $request->monto;
        $saldo_actual=$saldo_actual - $request->monto;

        DB::insert('INSERT INTO historial (tipo,monto,cuenta_destino,cod_user,descrip,fecha_hora,saldo_actual) values (?,?,?,?,?,?,?)', ['Transferencia',$request->monto,$request->cuenta,\Session::get('cod_user'),'Cuenta Origen: '.\Session::get('cuenta'),$hora_fecha,$saldo_actual]);
        ...
        DB::table('cuenta')
            ->where('num_cuenta',\Session::get('cuenta'))
            ->update(['saldo' => $saldo_actual]);            

        flash('TRANSFERENCIA EXITOSA: Se ha transferido correctamente el monto indicado')->success()->important();
        return redirect()->back();
    }
